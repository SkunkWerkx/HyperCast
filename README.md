...
- **Benchmarks across the whole spectrum** — each binding carries its ecosystem's own harness, HyperUuid-style: Criterion, BenchmarkDotNet, JMH, `testing.B`, pyperf, benchmark-ips, phpbench, and ordo-one's package-benchmark. The spine of the story, the RFC 3339 timestamp door vs. each platform's own parser (linux-arm64, first-wave numbers, crossing and transcode included in every HyperCast figure):
  ...
  | Ruby | 3.5 µs | 3.3 µs `Time.iso8601` | parity — atop a measured **1.6 µs raw Fiddle floor** |
  | Python (native backend) | **201 ns** | 163 ns `fromisoformat` (C-accelerated) | **near-parity** — see below |
  | Python (ctypes fallback) | 3.1 µs | — | the measured ~1 µs ctypes floor, kept as the universal/Pyodide path |
  | PHP | 2.9 µs | 1.3 µs `DateTimeImmutable` | loss (`modify()` in the wrapper, noted improvable; raw ext-ffi floor measured at ~105 ns — PHP's FFI is genuinely fast) |

  **Python's escape from the interpreted tier is its own receipt**: the losses were never "Python calling native code" — they were *ctypes* (interpreted marshalling, ~1 µs/call, measured). The `hypercast_native` backend is the Rust core linked directly into a CPython extension via PyO3 — no dlopen, no C-ABI hop, the same `METH_FASTCALL` door the builtins walk — and the package auto-selects it when importable, keeping pure ctypes as the zero-compile universal fallback (HyperUuid's Go dual-backend pattern, transplanted; `HYPERCAST_PURE=1` forces the fallback, and both backends replay the full corpus green). Result: every door 10–18x faster — timestamp 3.07 µs → **201 ns**, i32 → **146 ns** vs `int()`'s 88, uuid at parity with `uuid.UUID()` (both sides now bounded by `UUID.__init__` itself), and the forgiveness doors at ~180 ns for grammar the stdlib doesn't sell.

  The honest reading: the static tier (C#, JVM, Swift) wins by multiples wherever the platform runs culture machinery, because their FFI crossings are nanoseconds. The dynamic tier (Python, Ruby) loses per-call to its own FFI *mechanism* floor — measured per language, not guessed — and Go loses to a genuinely excellent stdlib plus cgo's tax. That floor is exactly what the round-three tabular layer amortizes: one crossing per chunk turns every row of this table into the native-loop numbers at the top. Benchmarking sagas worth knowing: the Ruby doors were 4.3 µs until per-call `Fiddle::Pointer.malloc` finalizers were hoisted to thread-local scratch, PHP read 20x slow until a loaded Xdebug was caught (`XDEBUG_MODE=off` for all recorded numbers), and Swift's first tape was pure measurement-floor quantization until `.kilo` scaling amortized it — receipts include their own forensics.
