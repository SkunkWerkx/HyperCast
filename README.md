...
- **The corpus is the contract** — `corpus/*.json` (seeded from the [Svartalfheim](https://github.com/NorseArchitecture/Svartalfheim) `Norse.Primitives` test suites this project descends from) replays through the Rust core's suite *and* every binding's. It also ships to a downstream .NET suite as the `HyperCast.Corpus` content package, versioned in lockstep, so a consumer replays the vectors its core was proven against instead of vendoring them. All eight replay the full thirteen-file set today — C# through real P/Invoke, Java through FFM downcalls, Ruby through *both* its Magnus and Fiddle backends.
