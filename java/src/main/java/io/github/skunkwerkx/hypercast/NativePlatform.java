...
/**
 * Maps the running JVM's OS/arch to the RID-style directory (matching the C# binding's
 ...
 * folders do — one jar has to work everywhere, so it bundles every platform's build under
 * {@code /native/{rid}/{filename}} and this picks the right one at runtime instead.
 */
final class NativePlatform {
    ...
    private static Target detect() {
        ...
        if (osName.contains("win")) {
            return new Target(isArm ? "win-arm64" : "win-x64", "hypercast.dll");
        }
        if (osName.contains("mac") || osName.contains("darwin")) {
            return new Target(isArm ? "osx-arm64" : "osx-x64", "libhypercast.dylib");
        }
        if (osName.contains("linux")) {
            return new Target(isArm ? "linux-arm64" : "linux-x64", "libhypercast.so");
        }
        throw new IllegalStateException(
                "Unsupported platform for hypercast: os.name=" + osName + " os.arch=" + osArch);
    }
